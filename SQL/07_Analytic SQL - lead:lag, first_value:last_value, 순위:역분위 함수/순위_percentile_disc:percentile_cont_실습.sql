...
-- 4분위별 sal 값을 반환. 
select percentile_disc(0.25) within group (order by sal) as qt_01, 
	   percentile_disc(0.5) within group (order by sal) as qt_02, 
	   percentile_disc(0.75) within group (order by sal) as qt_03, 
	   percentile_disc(1.0) within group (order by sal) as qt_04
from hr.emp;  


-- percentile_disc는 cume_dist의 inverse 값을 반환. 
-- percentile_disc는 0 ~ 1 사이의 분위수값을 입력하면 해당 분위수 값 이상인 것중에서 최소 cume_dist 값을 가지는 값을 반환
-- ex.  0.25를 넘는 최소값 -> 0.33 / 0.25 / 0.75 / 1 
