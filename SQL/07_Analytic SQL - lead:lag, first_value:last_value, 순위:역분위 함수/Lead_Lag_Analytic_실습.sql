...
select empno, deptno, hiredate, ename
	, lag(ename, 2) over (partition by deptno order by hiredate) as prev_ename
from hr.emp;
...
-- lag() over (order by desc)는 lead() over (order by asc)와 동일하게 동작
--order by 는 asc로 통일하는것이 좋음
select empno, deptno, hiredate, ename
